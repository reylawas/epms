...
if (isset($attendance_data[$date])) {
    ...
    foreach ($logs as $log) {
        // Check if the log time_in is before or after 12:00 PM
        $log_time_in = strtotime($log['time_in']);
        
        if ($log_time_in < strtotime('12:00 PM')) {
            // Morning log
            $time_in_am = date('h:i A', $log_time_in);
            // Check if time_out is 00:00:00 (or empty) and exclude it from undertime calculation
            $time_out_am = ($log['time_out'] == '00:00:00' || empty($log['time_out'])) ? null : date('h:i A', strtotime($log['time_out']));
            
            // Calculate undertime for AM, using time_in for undertime calculation if time_out is not available
            $undertime_am = calculateUndertime($log['time_in'], $time_out_am, '07:45 AM', '12:00 PM');
        } else {
            // Afternoon log
            $time_in_pm = date('h:i A', $log_time_in);
            // Check if time_out is 00:00:00 (or empty) and exclude it from undertime calculation
            $time_out_pm = ($log['time_out'] == '00:00:00' || empty($log['time_out'])) ? null : date('h:i A', strtotime($log['time_out']));
            
            // Calculate undertime for PM, using time_in for undertime calculation if time_out is not available
            $undertime_pm = calculateUndertime($log['time_in'], $time_out_pm, '12:45 PM', '04:45 PM');
        }
    }
}
